...
class LoginAction extends HomeBaseAction {
	...
	protected  function _send_to_resetpass($user){
		...
		//生成激活链接
		$url = $options['site_host'].U('user/login/password_reset',array("hash"=>$activekey), "", false);
		//邮件内容
		$template =<<<hello
		#username#，你好！<br>
		请点击或复制下面链接进行密码重置：<br>
...
hello;
    ...
    //登陆送积分
    public function signGetScore(){
        if($this->user['id']<1) $this->error('请登录',U('user/login/index'));
        $options_obj = D('Options');
        $site_options=  json_decode($options_obj->where("option_name='site_options'")->getField("option_value"));
        $signType = $site_options->signScore;
        if($signType==0){$this->error('已关闭签到功能');exit;}
        $now = time();
        $lastSignInfo = M('Sign')->where('uid='.$this->user['id'])->order('signTime DESC,id DESC')->find();
        $signTime = getdate($lastSignInfo['signTime']);
        $signTimeUnix=mktime(23,59,59,$signTime['mon'],$signTime['mday'],$signTime['year']);
        if($now<=$signTimeUnix){$this->error('您今天已经签到了，不要着急嘛，明天再来');exit;}
        switch($signType){
            case 1:
                $getScore = $site_options->signScoreNum;
                $msg = "签到成功，获得".$getScore."积分";
            break;
            case 2:
                $monthUnix = $this->getMonthLastDay($signTime['year'],$signTime['mon']);
                if($now<=$monthUnix){//同一个月，积分递增
                    $getScore = $lastSignInfo['getScore']+$site_options->signScoreNum;
                }else{//下一个月，初始化积分基数为0
                    $getScore = $site_options->signScoreNum;
                }
                $msg = "签到成功，本次获得".$getScore."积分，下次签到将获得".($getScore+$site_options->signScoreNum)."积分";
            break;
            case 3:
                $getScore = rand(0, $site_options->signScoreNum);
                if($getScore==0){
                    $msg = "连获得0积分这么攒人品的事都被你遇到了，看样这是快要中大奖的节奏啊";
                }else if($getScore>0 && $getScore<$site_options->signScoreNum){
                    $msg = "人品不错，恭喜你本次签到获得".$getScore."积分，再接再励";
                }else if($getScore==$site_options->signScoreNum){
                    $msg = "苍天呐，大地呐，你竟然不可思议的获得".$getScore."积分，真是人品爆发啊";
                }
            break;
        }
        ...
        $data['type'] = $signType;
        if(M('Sign')->add($data)){
            M('Users')->where('id='.$this->user['id'])->setInc('score',$getScore);
            $this->success($msg,"/");
        }
    }
    ...
    private function getMonthLastDay($year="",$month="") {
        ...
        return mktime(23,59,59,$month,$lastday,$year);
    }
    /**
     *  是否是闰年
     *  @return int
