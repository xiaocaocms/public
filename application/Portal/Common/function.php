 ...
 * @以字符串方式传入,通过sp_param_lable函数解析为以下变量
 * ids:调用指定id的一个或多个数据,如 1,2,3
 * field:调用goods指定字段,如(goods_id...) 默认全部
 * limit:数据条数,默认值为10,可以指定从第几条开始,如3,8(表示共调用8条,从第3条开始)
...
function sp_sql_goods($tag,$where=array()){
	...
	if (isset($tag['goods_id'])) {
		$where['goods_id'] = array('eq',$tag['goods_id']);
	}
	...
	$goods=$goods->field($field)->where($where)->order($order)->limit($limit)->select();
	return $goods;
}
...
/**
 * 功能：根据分类商品分类ID 获取该分类下所有商品（包含子分类中商品），已经分页，调用方式同sp_sql_goods_paged
 * param int $pagesize 分页数字.
 * param string $pagetpl 以字符串方式传入,例："{first}{prev}{liststart}{list}{listend}{next}{last}"
 */

function sp_sql_goods_paged_bycatid($cid,$tag,$pagesize=20,$pagetpl='{first}{prev}{liststart}{list}{listend}{next}{last}'){
	$catIDS=array();
	$terms=M("Terms")->field("term_id")->where("status=1 and (term_id=$cid or path like '%-$cid-%')")->order('term_id asc')->select();
	foreach($terms as $item){
		$catIDS[]=$item['term_id'];
	...
	$content= sp_sql_goods_paged($catIDS.$tag,$pagesize,$pagetpl);
	return $content;
}
