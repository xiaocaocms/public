...
class Page {
    ...
    private function UrlParameters($url = array()) {
        ...
        if ($this->Static) {
        	...
        	if($this->searching){
        		$url=leuu(GROUP_NAME  . "/" . MODULE_NAME . "/" . ACTION_NAME)."?".http_build_query ($arg);
        	}else{
        		$url=leuu(GROUP_NAME  . "/" . MODULE_NAME . "/" . ACTION_NAME,$arg);
        	}
    ...
    public function Pager($Page_tpl = '') {
        ...
        if ($cfg ['pageindex'] > 1) {
            ...
            if ($this->Static && ($cfg ['pageindex'] - 1) == 1) {
                $pPrev = $this->linkwraper_pre.'<a class="pg-prev" href="' . $this->PageLink['index'] . '">' . $cfg ['prev'] . '</a> '.$this->linkwraper_after; //上一页
            } else {
                $pPrev = $this->linkwraper_pre.'<a class="pg-prev" href="' . str_replace('*', $cfg ['pageindex'] - 1, $cfg ['link']) . '">' . $cfg ['prev'] . '</a> '.$this->linkwraper_after;

            }
        }else{
            $pPrev = $this->linkwraper_pre.'<span class="pg-prev">上一页</span> '.$this->linkwraper_after;
        }
        //下一页，尾页
        if ($cfg ['pageindex'] < $cfg ['pagecount']) {
            ...
            $pNext = $this->linkwraper_pre.'<a class="pg-next" href="' . str_replace('*', $cfg ['pageindex'] + 1, $cfg ['link']) . '">' . $cfg ['next'] . '</a> '.$this->linkwraper_after;
        }else{
            $pPrev = $this->linkwraper_pre.'<span class="pg-prev">上一页</span> '.$this->linkwraper_after.'<span class="pg-next">下一页</span> ';
        }
